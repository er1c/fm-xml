...
"""
  ...
  private val simplePartOneLineXml =
...
"""
  ...
  private val simplePartOneLineExtraClosingTagXml =
    """<?xml version='1.0' encoding='UTF-8'?>
...
""".trim
  ...
  private val simplePartOneLineXmlWithDTD =
    """<?xml version='1.0' encoding='UTF-8'?><!DOCTYPE ACES SYSTEM "ACES.dtd"><feed><part><uniqueId>ONE</uniqueId><source>foo</source><name>Simple Part One</name></part><price><uniqueId>ONE</uniqueId><price>9.99</price></price><part><uniqueId>TWO</uniqueId><source>foo</source><name>Simple Part Two</name></part><price><uniqueId>TWO</uniqueId><price>19.99</price></price></feed>""".trim
...
"""
  ...
  test("Feed that has elements but not what we are looking for") {
    val reader =
      makeReader(nestedPartXml, paths = Seq(NestedItemsPartElementNotExistPath, NestedItemsPriceElementNotExistPath))
    reader.hasNext should equal(false)
  }
  ...
  val NestedItemsPartElementNotExistPath = XmlReaderPath[SimpleFeedPart, MyPathValue](
    "items/element_that_does_not_exist",
    NestedItemsPartElementNotExistPathValue(_))
  val NestedItemsPriceElementNotExistPath = XmlReaderPath[SimpleFeedPrice, MyPathValue](
    "prices/element_that_does_not_exist",
    NestedItemsPriceElementNotExistPathValue(_))

  val DeepNestedPartPath =
    XmlReaderPath[SimpleFeedPart, MyPathValue]("items/items2/items3/part", DeepNestedPartPathValue(_))
  val DeepNestedPricePath =
    XmlReaderPath[SimpleFeedPrice, MyPathValue]("items/items2/items3/price", DeepNestedPricePathValue(_))
  ...
  private def checkValue(
    ...
    value match {
      ...
      case NestedItemsPriceElementNotExistPathValue(price: SimpleFeedPrice) =>
        checkPrice(price, uniqueId, expectedPrice)
  ...
  private def makeReader(
    xml: String,
    root: String = "feed",
    paths: Seq[XmlReaderPath[_, MyPathValue]] = Seq(PartPath, PricePath)): Iterator[MyPathValue] = {
