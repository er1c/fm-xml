...
object XmlReaderPath {
  ...
  def apply[XmlValue: ClassTag](itemPath: String): XmlReaderPath[XmlValue, XmlValue] =
    XmlReaderPath[XmlValue, XmlValue](itemPath, identity)
...
final case class XmlReaderPath[XmlValue: ClassTag, MappedXmlValue](
  ...
  private[woodstox] val path: Array[String] = itemPath.split('/')

  // The name of the element we care about (last part of the path)
  // items/part => part
  private[woodstox] val itemName: String = path.last

  private[woodstox] val targetDepth: Int = path.length

  private[woodstox] def readValue(xmlStreamReader: XMLStreamReader2): MappedXmlValue = {
   ...
   * @return
   */
  private[woodstox] def getMatchType(currentPath: IndexedSeq[String]): XmlReaderPathMatchType = {
