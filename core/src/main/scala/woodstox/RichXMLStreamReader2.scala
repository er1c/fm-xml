...
object RichXMLStreamReader2 {
  implicit def toRichXMLStreamReader2(sr: org.codehaus.stax2.XMLStreamReader2): RichXMLStreamReader2 =
    new RichXMLStreamReader2(sr)
}
...
final class RichXMLStreamReader2(val sr: XMLStreamReader2) extends AnyVal {
  ...
  def requireNot(tpe: Int): Unit = {
    if (sr.getEventType() === tpe)
      throw new XMLStreamException(s"Expected current tag to be NOT $tpe", sr.getLocation())
  }

  def requireEither(one: Int, two: Int): Unit = {
    val tpe: Int = sr.getEventType()
    if (one != tpe && two != tpe)
      throw new XMLStreamException(s"Expected current tag to be either $one or $two", sr.getLocation())
  }
  ...
  def seekToSiblingElement(name: String): Unit = {
    if (!trySeekToSiblingElement(name))
  ...
  def seekToEndOfParentElement(): Unit = {
    ...
    require(
      sr.getDepth() === targetDepth,
      s"Something failed:  Target Depth: $targetDepth  Current Depth: ${sr.getDepth()}")
    ...
    while (!done && sr.hasNext) {
      ...
      if (tpe === START_ELEMENT) {
        ...
        if (sr.getLocalName() != pathParts(pathIdx)) sr.skipElement()
          ...
          require(
            sr.getDepth === depth,
            s"Invalid Usage of Foreach.  Expected to end at the same depth we started!  Depth before calling f: $depth   Depth after calling f:  ${sr.getDepth}"
          )
        }
