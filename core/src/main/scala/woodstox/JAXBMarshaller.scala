...
final class JAXBMarshaller[T: ClassTag](
  ...
  private[this] val context: JAXBContext = {
    ...
    try {
      ...
      case ex: Exception =>
        logger.error(
          s"Caught exception trying to create JAXBContext for Jaxb2Marshaller($packageName) with classes: ${classes.map { _.getName }
            .mkString(", ")}",
          ex)
        throw ex
    }
  ...
  def toXML(obj: T, comments: XMLCommentProvider): String = {
    ...
    Resource.using(outputFactory.createXMLStreamWriter(writer)) { writer: XMLStreamWriter =>
      val wrapped: CommentingXMLStreamWriter =
        new CommentingXMLStreamWriter(new IndentingXMLStreamWriter(writer), comments)
  ...
  def writeXML(obj: T, os: OutputStream, encoding: String): Unit =
    Resource.using(outputFactory.createXMLStreamWriter(os, encoding)) { writeXML(obj, _) }
  ...
  def readXML(is: InputStream, encoding: String): T =
    Resource.using(inputFactory.createXMLStreamReader(is, encoding)) { readXML }
