...
final class XmlWriter(classes: Seq[Class[_]], rootName: String, defaultNamespace: String, outputStream: OutputStream)
  extends Closeable with Validatable {
  ...
  private[this] val xmlStreamWriter2: XMLStreamWriter2 =
    outputFactory.createXMLStreamWriter(outputStream, encoding).asInstanceOf[XMLStreamWriter2]
  ...
  def setValidationProblemHandler(h: ValidationProblemHandler): ValidationProblemHandler =
    validatable.setValidationProblemHandler(h)
