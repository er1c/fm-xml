...
import woodstox.RichXMLStreamReader2.toRichXMLStreamReader2
...
object XmlReader {
  ...
  def apply[T: ClassTag](
    ...
    resource: Resource[Reader]): XmlReader[T] = {
  ...
  def apply[T](
    ...
    targetHead: XmlReaderPath[_, T],
    targetsRest: XmlReaderPath[_, T]*): XmlReader[T] = {
   ...
   */
  private class DefaultNamespaceStreamReaderDelegate(self: XMLStreamReader2, defaultNamespaceURI: String)
    extends StreamReader2Delegate(self) {
   ...
   */
  private class OverrideDefaultNamespaceStreamReaderDelegate(
    self: XMLStreamReader2,
    overrideDefaultNamespaceURI: String)
    extends StreamReader2Delegate(self) {
...
final case class XmlReader[T](
  ...
  if (defaultNamespaceURI.isNotNullOrBlank)
    require(
      overrideDefaultNamespaceURI.isNullOrBlank,
      "Can't set both defaultNamespaceURI and overrideDefaultNamespaceURI")
  if (overrideDefaultNamespaceURI.isNotNullOrBlank)
    require(defaultNamespaceURI.isNullOrBlank, "Can't set both defaultNamespaceURI and overrideDefaultNamespaceURI")

  private def wrapXMLStreamReader2(r: XMLStreamReader2): XMLStreamReader2 = {
    if (overrideDefaultNamespaceURI.isNotNullOrBlank)
      new XmlReader.OverrideDefaultNamespaceStreamReaderDelegate(r, overrideDefaultNamespaceURI)
    else if (defaultNamespaceURI.isNotNullOrBlank)
      new XmlReader.DefaultNamespaceStreamReaderDelegate(r, defaultNamespaceURI)
    else r
  }
  ...
  protected def foreachWithResource[U](f: T => U, reader: Reader): Unit = {
    ...
    Resource.using(
      wrapXMLStreamReader2(
        inputFactory.createXMLStreamReader(XmlInvalidCharFilter(reader)).asInstanceOf[XMLStreamReader2])) {
