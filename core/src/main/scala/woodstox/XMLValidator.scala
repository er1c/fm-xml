...
object XMLValidator {
  ...
  final class ValidationResultBuilder(limit: Option[Int]) extends ValidationProblemHandler {
    ...
    def reportProblem(problem: XMLValidationProblem): Unit = {
      if (limit.isEmpty || validationErrorCount < limit.get)
        validationErrors += ErrorMessage(problem.getMessage, problem.getLocation)
  ...
  private implicit def toLocation(loc: javax.xml.stream.Location): Location =
    Location(loc.getLineNumber, loc.getColumnNumber, loc.getCharacterOffset)
  ...
  def createXMLValidationSchema(xsd: File): XMLValidationSchema =
    InputStreamResource.forFileOrResource(xsd).reader().use { createXMLValidationSchema }
  ...
  def createXMLValidationSchema(xsd: Reader): XMLValidationSchema = {
    val sf: XMLValidationSchemaFactory =
  ...
  def validate(xsd: File, xml: File, limit: Option[Int]): ValidationResult =
    Resource
      .use(InputStreamResource.forFileOrResource(xsd).reader(), InputStreamResource.forFileOrResource(xml).reader()) {
        validate(_, _, limit)
      }
  ...
  def validate(xsd: Resource[Reader], xml: Resource[Reader], limit: Option[Int]): ValidationResult =
    Resource.use(xsd, xml) { validate(_, _, limit) }
  def validate(xsd: Reader, xml: Reader, limit: Option[Int]): ValidationResult =
    validate(createXMLValidationSchema(xsd), xml, limit)
  ...
  def validate(xsd: XMLValidationSchema, xml: Reader, limit: Option[Int]): ValidationResult = {
    ...
    SingleUseResource(inputFactory.createXMLStreamReader(xml).asInstanceOf[XMLStreamReader2]).use {
      sr: XMLStreamReader2 =>
        ...
        try {
          ...
          case _: XMLValidationException => resultBuilder.result()
        }
    }
  }
